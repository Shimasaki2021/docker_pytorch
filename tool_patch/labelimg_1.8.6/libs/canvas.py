...
class Canvas(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.current is not None and len(self.line) == 2:
            ...
            p.drawRect(int(left_top.x()), int(left_top.y()), int(rect_width), int(rect_height)) # bugfix 2024.12.28

        if self.drawing() and not self.prev_point.isNull() and not self.out_of_pixmap(self.prev_point):
            p.setPen(QColor(0, 0, 0))
            #p.drawLine(self.prev_point.x(), 0, self.prev_point.x(), self.pixmap.height())
            #p.drawLine(0, self.prev_point.y(), self.pixmap.width(), self.prev_point.y())
            p.drawLine(int(self.prev_point.x()), 0, int(self.prev_point.x()), self.pixmap.height()) # bugfix 2024.12.28
            p.drawLine(0, int(self.prev_point.y()), self.pixmap.width(), int(self.prev_point.y()))  # bugfix 2024.12.28
